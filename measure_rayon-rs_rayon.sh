#!/bin/bash
# 测量 rayon-rs/rayon 最小硬件：真实执行 build + test，进程树峰值监控（覆盖多线程/多进程）
set -euo pipefail

REPO_NAME="rayon-rs_rayon"
...
cleanup() {
  ...
  rm -rf "$WORK_DIR"
  rm -f "$RESULT_DIR/rayon_build" "$RESULT_DIR/rayon_build.peaks" \
        "$RESULT_DIR/rayon_run" "$RESULT_DIR/rayon_run.peaks"
}
trap cleanup EXIT
...
echo "=== [2/5] Build 阶段：cargo build -p rayon -p rayon-core --tests ===" | tee -a "$LOG_FILE"
BUILD_START=$(date +%s)
set +e
cargo build -p rayon -p rayon-core --tests 2>&1 &
BUILD_PID=$!
BUILD_PGID=$(ps -o pgid= -p "$BUILD_PID" 2>/dev/null | tr -d ' ')
"$MONITOR" "$BUILD_PID" "$RESULT_DIR/rayon_build" "$BUILD_PGID" &
MON1_PID=$!
wait "$BUILD_PID"
BUILD_EXIT=$?
...
echo "=== [3/5] Run 阶段：cargo test -p rayon -p rayon-core ===" | tee -a "$LOG_FILE"
RUN_START=$(date +%s)
set +e
cargo test -p rayon -p rayon-core 2>&1 &
RUN_PID=$!
RUN_PGID=$(ps -o pgid= -p "$RUN_PID" 2>/dev/null | tr -d ' ')
"$MONITOR" "$RUN_PID" "$RESULT_DIR/rayon_run" "$RUN_PGID" &
MON2_PID=$!
wait "$RUN_PID"
RUN_EXIT=$?
...
BUILD_PEAK_GB=$(awk -F= '/^PEAK_RAM_GB=/{print $2}' "$RESULT_DIR/rayon_build.peaks" | tr -d '[:space:]')
RUN_PEAK_GB=$(awk -F= '/^PEAK_RAM_GB=/{print $2}' "$RESULT_DIR/rayon_run.peaks" | tr -d '[:space:]')
...
cat > "$RESULT_DIR/${REPO_NAME}.json" << EOF
{
  ...
  "url": "${REPO_URL}",
  "mvw": {
    "mvw_command": "cargo build -p rayon -p rayon-core --tests && cargo test -p rayon -p rayon-core",
    "build_command": "cargo build -p rayon -p rayon-core --tests",
    "run_command": "cargo test -p rayon -p rayon-core",
    "input_scale": "core crates tests (rayon + rayon-core), excludes rayon-demo nightly-only test target",
  ...
  "static_triage": {
    ...
    "notes": "Rust data-parallel library; no GPU requirement; compile and tests are CPU/RAM intensive."
  },
  "peaks_observed": {
    ...
    "cpu_utilization_summary": "build high (cargo compiling many crates in parallel), run moderate-high (parallel tests and Rayon thread pool)"
  },
  "margins_used": "none",
  "min_hardware": {
    "build_min_spec": {
