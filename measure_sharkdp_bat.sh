#!/bin/bash
# 测量 sharkdp/bat 最小硬件：真实执行 build + run，进程树峰值监控（覆盖多线程/多进程）
set -euo pipefail

REPO_NAME="sharkdp_bat"
...
if [ ! -x "$WORK_DIR/target/release/bat" ]; then
  echo "未找到 target/release/bat，Build 结果无效" | tee -a "$LOG_FILE"
  exit 1
fi

echo "=== [3/5] Run 阶段：执行 bat 渲染文件（重复多次以稳定采样峰值） ===" | tee -a "$LOG_FILE"
printf 'bat minimal hardware measurement\nline2\nline3\n' > "$WORK_DIR/mvw_input.txt"
RUN_START=$(date +%s)
set +e
(
  cd "$WORK_DIR"
  for _ in $(seq 1 200); do
    ./target/release/bat --paging=never --color=never mvw_input.txt >/dev/null
  done
) 2>&1 | tee -a "$LOG_FILE" &
RUN_PID=$!
...
cat > "$RESULT_DIR/${REPO_NAME}.json" << EOF
{
  ...
  "url": "${REPO_URL}",
  "mvw": {
    "mvw_command": "cargo build --release && for i in \$(seq 1 200); do ./target/release/bat --paging=never --color=never mvw_input.txt >/dev/null; done",
    "build_command": "cargo build --release",
    "run_command": "for i in \$(seq 1 200); do ./target/release/bat --paging=never --color=never mvw_input.txt >/dev/null; done",
    "input_scale": "single local text file (3 lines), repeated 200 invocations",
    "success_criteria": "both commands exit code 0 and target/release/bat exists"
  },
  "environment": {
    "os": "Linux",
