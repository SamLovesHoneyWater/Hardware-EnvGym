#!/bin/bash
# 测量 tokio-rs/bytes 最小硬件：真实执行 build + test，进程树峰值监控（覆盖多线程/多进程）
set -euo pipefail

REPO_NAME="tokio-rs_bytes"
...
cleanup() {
  ...
  rm -rf "$WORK_DIR"
  rm -f "$RESULT_DIR/bytes_build" "$RESULT_DIR/bytes_build.peaks" \
        "$RESULT_DIR/bytes_run" "$RESULT_DIR/bytes_run.peaks"
}
trap cleanup EXIT
...
echo "=== [2/5] Build 阶段：cargo build --tests --jobs 1 ===" | tee -a "$LOG_FILE"
BUILD_START=$(date +%s)
set +e
cargo build --tests --jobs 1 2>&1 &
BUILD_PID=$!
BUILD_PGID=$(ps -o pgid= -p "$BUILD_PID" 2>/dev/null | tr -d ' ')
"$MONITOR" "$BUILD_PID" "$RESULT_DIR/bytes_build" "$BUILD_PGID" &
MON1_PID=$!
wait "$BUILD_PID"
BUILD_EXIT=$?
...
echo "=== [3/5] Run 阶段：cargo test --jobs 1 -- --test-threads=1 ===" | tee -a "$LOG_FILE"
RUN_START=$(date +%s)
set +e
cargo test --jobs 1 -- --test-threads=1 2>&1 &
RUN_PID=$!
RUN_PGID=$(ps -o pgid= -p "$RUN_PID" 2>/dev/null | tr -d ' ')
"$MONITOR" "$RUN_PID" "$RESULT_DIR/bytes_run" "$RUN_PGID" &
MON2_PID=$!
wait "$RUN_PID"
RUN_EXIT=$?
...
BUILD_PEAK_GB=$(awk -F= '/^PEAK_RAM_GB=/{print $2}' "$RESULT_DIR/bytes_build.peaks" | tr -d '[:space:]')
RUN_PEAK_GB=$(awk -F= '/^PEAK_RAM_GB=/{print $2}' "$RESULT_DIR/bytes_run.peaks" | tr -d '[:space:]')
...
cat > "$RESULT_DIR/${REPO_NAME}.json" << EOF
{
  ...
  "url": "${REPO_URL}",
  "mvw": {
    "mvw_command": "cargo build --tests --jobs 1 && cargo test --jobs 1 -- --test-threads=1",
    "build_command": "cargo build --tests --jobs 1",
    "run_command": "cargo test --jobs 1 -- --test-threads=1",
    "input_scale": "bytes 仓库默认测试集，单 job 构建 + 单线程测试",
  ...
  "static_triage": {
    ...
    "notes": "Rust bytes library; no GPU requirement; build/test mainly consume CPU, RAM, and disk."
  },
  "peaks_observed": {
    ...
    "cpu_utilization_summary": "build moderate-high (cargo compile), run moderate (unit/doc/integration tests)"
  },
  "margins_used": "none",
  "min_hardware": {
    "build_min_spec": {
